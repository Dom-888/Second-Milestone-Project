...
let output = []; // At the beginning of the game it's composed by a row of underscores, which will be gradually replaced by the letters guessed by the player. This variable is printed in the hiddenWord ID in HTML.
...
const game = {
    ...
    // Check if the letter chosen by the user is part of the word to guess
    ...
    updateTopScores: function (newScore) {
        ...
        if (localStorage.first == undefined) { localStorage.first = newScore; }
            ...
            if (localStorage.second != undefined) { localStorage.third = localStorage.second; }
